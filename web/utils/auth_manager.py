...
class AuthManager:
    ...
    def register_wechat_user(self, wechat_info: Dict) -> Tuple[bool, Optional[Dict], str]:
        """
        注册微信登录用户
        
        Args:
            wechat_info: 微信用户信息字典，包含openid, nickname等
            
        Returns:
            (是否成功, 用户信息字典, 消息)
        """
        if not wechat_info or "openid" not in wechat_info:
            return False, None, "微信用户信息不完整"
        
        openid = wechat_info["openid"]
        nickname = wechat_info.get("nickname", f"微信用户_{openid[:8]}")
        
        # 生成用户名：使用wechat_前缀 + openid的一部分，确保唯一性
        username = f"wechat_{openid[:12]}"
        
        users = self._load_users()
        
        # 检查用户是否已存在（通过openid或用户名）
        existing_user = None
        for uname, uinfo in users.items():
            if uinfo.get("wechat_openid") == openid:
                existing_user = uname
                break
        
        if existing_user:
            # 用户已存在，更新信息
            user_info = users[existing_user]
            user_info["wechat_nickname"] = nickname
            user_info["wechat_headimgurl"] = wechat_info.get("headimgurl", "")
            user_info["last_login_time"] = time.time()
            
            if self._save_users(users):
                logger.info(f"✅ 微信用户信息已更新: {existing_user}")
                return True, {
                    "username": existing_user,
                    "role": user_info.get("role", "user"),
                    "permissions": user_info.get("permissions", ["analysis"]),
                    "points": int(user_info.get("points", 10)),
                    "wechat_openid": openid,
                    "wechat_nickname": nickname
                }, "登录成功"
            else:
                return False, None, "更新用户信息失败"
        
        # 新用户，自动注册
        # 生成随机密码（微信登录用户不需要密码，但为了系统兼容性，生成一个随机密码）
        random_password = secrets.token_urlsafe(16)
        
        users[username] = {
            "password_hash": self._hash_password(random_password),  # 随机密码，实际不使用
            "role": "user",  # 默认普通用户
            "permissions": ["analysis"],  # 默认只有分析权限
            "points": 10,  # 默认10点
            "created_at": time.time(),
            "created_by": "wechat_login",  # 标记为微信登录创建
            "wechat_openid": openid,
            "wechat_unionid": wechat_info.get("unionid", ""),
            "wechat_nickname": nickname,
            "wechat_headimgurl": wechat_info.get("headimgurl", ""),
            "last_login_time": time.time()
        }
        
        if self._save_users(users):
            logger.info(f"✅ 微信用户自动注册成功: {username} ({nickname})")
            return True, {
                "username": username,
                "role": "user",
                "permissions": ["analysis"],
            ...
            return False, None, "保存用户信息失败"
    
    def login_wechat_user(self, wechat_info: Dict) -> Tuple[bool, Optional[Dict], str]:
        """
        微信用户登录
        
        Args:
            wechat_info: 微信用户信息字典
            
        Returns:
            (是否成功, 用户信息字典, 消息)
        """
        # 先尝试注册/更新用户
        success, user_info, message = self.register_wechat_user(wechat_info)
        ...
        st.session_state.user_info = user_info
        st.session_state.login_time = time.time()
        st.session_state.login_method = "wechat"  # 标记登录方式
        ...
        save_to_cache_js = f"""
        <script>
        console.log('🔐 保存微信登录认证数据到localStorage');
        try {{
            const authData = {json.dumps(auth_data)};
            localStorage.setItem('tradingagents_auth', JSON.stringify(authData));
            console.log('✅ 微信登录认证数据已保存');
        }} catch (e) {{
            console.error('❌ 保存认证数据失败:', e);
        }}
        </script>
        ...
        if user_activity_logger:
            user_activity_logger.log_login(username, True, login_method="wechat")
        
        logger.info(f"✅ 微信用户登录成功: {username}")
